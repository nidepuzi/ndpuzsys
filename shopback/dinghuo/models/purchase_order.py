...
class OrderList(models.Model):
    ...
    def related_inbounds(self):
        if not hasattr(self, '_related_inbounds_'):
            from shopback.dinghuo.models.inbound import InBound
    ...
    def related_out_stock_inbound_details(self):
        if not hasattr(self, '_related_out_stock_inbound_details_'):
            from shopback.dinghuo.models.inbound import InBoundDetail
    ...
    def purchase_order(self):
        from shopback.dinghuo.models_purchase import PurchaseOrder
    ...
    def to_excel_data(self, format):
        ...
        if not format:
            ...
            columns = [u'订单号', u'产品条码', u'订单状态', u'买家id', u'子订单编号', u'供应商编码', u'买家昵称', u'商品名称', u'产品规格', u'商品单价',
                       u'商品数量',
                       u'商品总价', u'运费', u'购买优惠信息', u'总金额', u'买家购买附言', u'收货人姓名', '身份证号码', u"身份证图像链接",u"身份证反面图像链接",u'省', u'市', u'区/县', u'收货详细地址',u'邮编',
                       u'收货人手机', u'收货人电话', u'买家选择运送方式', u'卖家备忘内容', u'订单创建时间', u'付款时间', u'物流公司', u'物流单号', u'发货附言',
                       u'发票抬头', u'电子邮件', u'商品链接']
            for p in need_send:
                o = p.package_order
                saleproduct = p.product_sku.product.get_sale_product()
                ua = UserAddress.objects.filter(id=o.user_address_id)
                idcard_url = '用户未上传个人身份证'
                back_idcard_url = '用户未上传个人身份证'
                for i in ua:
                    if i.extras:
                        try:
                            idcard_url = generate_private_url(i.extras.get(u"idcard").get("face"))
                            back_idcard_url = generate_private_url(i.extras.get(u"idcard").get("back"))
                        except:
                            pass
                items.append(
                    [str(o.pid) if o else '', '', p.get_assign_status_display(), str(p.sale_trade.buyer_id), str(p.id),
                     ...
                     '\'' + str(o.user_address.idcard_no) if o and o.user_address else '',idcard_url,back_idcard_url,
            ...
            for p in need_send.exclude(package_order_pid=None):
                ...
                items.append([str(p.id), p.book_time.strftime('%Y-%m-%d %H:%M:%S'), p.pay_time.strftime('%Y-%m-%d %H:%M:%S'),
                              u'货到付款', u'你的铺子，时尚健康美丽', '', str(p.num * p.product_sku.cost),
            ...
            columns = [u'订单号', u'产品条码', u'订单状态', u'买家id', u'子订单编号', u'供应商编码',  u'供应商sku编码', u'买家昵称', u'商品名称', u'产品规格', u'商品单价',
                       u'商品数量',
                       u'商品总价', u'运费', u'购买优惠信息', u'总金额', u'买家购买附言', u'收货人姓名', u'收货地址', u'邮编',
                       u'收货人手机', u'收货人电话', u'买家选择运送方式', u'卖家备忘内容', u'订单创建时间', u'付款时间', u'物流公司', u'物流单号', u'发货附言',
                       u'发票抬头', u'电子邮件', u'商品链接']
...
def order_list_update_stage(sender, instance, created, **kwargs):
    ...
    if instance.stage in [OrderList.STAGE_PAY, OrderList.STAGE_RECEIVE, OrderList.STAGE_STATE]:
        from shopback.dinghuo.tasks import task_orderlist_update_self
...
#
#         from shopback.dinghuo.tasks import task_update_purchasedetail_status, \
...
#         from shopback.forecast.apis import api_create_or_update_forecastinbound_by_orderlist
#         try:
...
#     from shopback.forecast.models import ForecastInbound
#     from shopback.forecast import tasks
...
def update_orderlist(sender, instance, created, **kwargs):
    from shopback.dinghuo.tasks import task_orderdetail_update_orderlist
