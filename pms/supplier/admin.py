...
class SaleSupplierAdmin(ApproxAdmin):
    list_display = ('id', 'supplier_code', 'vendor_code', 'supplier_name_link', 'charge_link',
                    ...
                    'supplier_type_choice', 'supplier_zone_choice', 'address', 'ware_by', 'return_ware_by', 'memo_well',"can_change_logistics")
    ...
    def charge_link(self, obj):
        if obj.status == SaleSupplier.CHARGED:  # 如果是已经接管
            ...
            return u'<a href="/pms/supplier/product/?status=purchase&sale_supplier={0}"' \
...
class SaleProductAdmin(ApproxAdmin):
    ...
    def outer_id_link(self, obj):
        ...
        if obj.status in (SaleProduct.SELECTED, SaleProduct.PASSED, SaleProduct.PURCHASE,
            ...
            # test_link += u'<br><br><a href="/pms/supply/sample/add_sample/?outer_id={0}&title={1}&pic_url={2}&sale_supplier={3}&sale_price={4}&std_sale_price={5}" class="btn" target="_blank" >{6}</a>'
            #
            ...
            test_link += u'<br><br><a href="/apis/items/v1/product?supplier_id={0}&saleproduct={1}" class="btn" target="_blank" >{2}</a>' \
                         u'<a href="/pms/supplier/bdproduct/{1}/" class="btn" target="_blank" >{3}</a>'
...
#                 rset.remove(contactor_name)
    ...
    def supplier_link(self, obj):
        ...
        if obj.status in (SaleProduct.SELECTED, SaleProduct.PURCHASE, SaleProduct.WAIT, SaleProduct.PASSED,
            ...
            base_link += u'<br><br><a href="/pms/supplier/product/?status={0}&sale_supplier={1}"  target="_blank" >{2}</a></div>'
